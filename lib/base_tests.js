...
describe('Harness initialization', function() {
  ...
  it('should have a ui require statement if ui code is present', function () {
    let hasUiLib = false;
    try {
      fs.readFileSync(path.join(".", "lib", "ui", "index.js")); // not used, just reading to throw if it's not found
      hasUiLib = true;
    }
    catch(e) {
      assert.equal(e.code, "ENOENT", "test only throws when no lib/ui/index.js found");
    }

    if(hasUiLib) {
      // assert this outside the try() above because it also throws on failure
      assert.isDefined(thisModule.ui, "module has code in 'lib/ui' but does not define 'ui' in 'index.js'");
    }
  });
});
    ...
    describe('Validate function', function () {
      it('should throw if it has undefined envVariables', function () {
        if (integration.envVariables.length > 0) {
          ...
          assert.throws(invoke, (/Missing credentials, contact ActiveProspect Support/i));
        }
      });
